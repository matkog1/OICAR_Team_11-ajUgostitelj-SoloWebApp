...
public class CategoriesApiClientIntegrationTests : IntegrationTestBase
{
    ...
    {
        // CREATE
        var name0 = $"Integracijski-test{Guid.NewGuid()}";
        var created = await _client.CreateCategoryAsync(new CategoryDto { Name = name0, ProductCount = 69});
        Assert.NotNull(created);
        ...
        try
        {
            ...
            Assert.Equal(name0, fetched!.Name);

            // update
            var name1 = name0 + "update test";
            created.Name = name1;
